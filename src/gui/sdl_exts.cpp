...
void SDL_RenderDrawBitmap(
    SDL_Renderer *renderer, FT_Bitmap bitmap, int x, int y
) {
    ...
    if (surface == nullptr) {
        std::cout << "error: Couldn't make glyph surface - " << SDL_GetError()
                  << std::endl;
        exit(-1);
    }
...
void SDL_RenderDrawChar(
    ...
    SDL_RenderDrawBitmap(
        renderer, face->glyph->bitmap, calculated_x, calculated_y
    );
}
...
void SDL_RenderDrawText(
    ...
    for (auto c : text) {
        ...
        default:
            SDL_RenderDrawChar(
                renderer, face, c, fontsize, x + offset_x, y + offset_y
            );
            offset_x += offset_step;
            break;
        }
